...
public class Space
{
	private HashMap<Point, BaseCell> cells;
	private Point size;

	public Space(HashSet<BaseCell> cells, Point size) throws SpaceCreationException
	{
	...
	private HashMap<Point, BaseCell> buildDict(HashSet<BaseCell> cells)
	{
		HashMap<Point, BaseCell> result = new HashMap<>();
		for (BaseCell c : cells)
			result.put(c.getLocation(), c);
		return result;
	}

	public GameField getSection(Point p)
	{
		if (p.getX() != 0)
		{
			...
			for (int y = 0; y < cells.length; y++)
				for (int z = 0; z < cells[0].length; z++)
					cells[y][z] = this.cells.get(new Point(p.getX(), y, z));
		...
		} else if (p.getY() != 0)
		{
			...
			for (int z = 0; z < cells.length; z++)
				for (int x = 0; x < cells[0].length; x++)
					cells[z][x] = this.cells.get(new Point(x, p.getY(), z));
			return new GameField(cells);
		} else
		{
			...
			for (int x = 0; x < cells.length; x++)
				for (int y = 0; y < cells[0].length; y++)
					cells[x][y] = this.cells.get(new Point(x, y, p.getZ()));
			return new GameField(cells);
		}
	}
}
